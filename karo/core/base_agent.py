...
class BaseAgentConfig(BaseModel):
    ...
    provider_config: Union[OpenAIProviderConfig, AnthropicProviderConfig] = Field(..., description="Configuration object for the LLM provider (e.g., OpenAIProviderConfig). Must include a 'type' field.", discriminator='type')
...
class BaseAgent:
    ...
    def __init__(self, config: BaseAgentConfig):
        ...
        if provider_config.type == 'openai':
            from karo_copy.karo.providers.openai_provider import OpenAIProvider
            self.provider = OpenAIProvider(config=provider_config)
        elif provider_config.type == 'anthropic':
            from karo_copy.karo.providers.anthropic_provider import AnthropicProvider
        ...
        if config.memory_manager_config:
            ...
            from karo_copy.karo.memory.memory_manager import MemoryManager # Import locally
            try:
